...
void init_shell_display(){
    ...
    printf("***********************************************************");
    printf("***********************************************************\n\n");
    printf("\tWELCOME TO Rajat'S SHELL\n\n");
...
int cd(int no_of_args,char **args)
{
    ...
    if(!strcmp(args[1],"-")){   //change to previous directory
        args[1]="..";
    }
	int dir_exists;
	dir_exists = isDir(args[1]);		
	if (dir_exists==1){
		chdir( args[1] );      // change directory to the specified directory             
		return 1;                          
	}       
...
char **argumentSeparation(char *inputStr){   //split 
...
int main(int argc, char **argv){
	...
	while(flag){
        ...
        if((strcmp(args[1],"&t")))  // if not thread
        {
            pid_t pid,wpid;  // process id  , wait process id
            ...
            {
                char comfile[SIZE];   // command file
            ...
            {
                wpid=waitpid(pid,NULL, WUNTRACED);  // wait for the child process to terminate
                if(wpid<0)
                {
